...
app.post("/api/agent/fix", async (req, res) => {
  try {
    ...
    if (!issueUrl) {
      return res.status(400).json({ error: "issueUrl 不能为空" });
    }
    ...
    res.json({
      success: true,
      sessionId,
      state: session?.state || 'perceiving',
      message: '修复流程已启动',
    });
  } catch (error: any) {
    console.error("[Agent Fix] Error:", error);
    res.status(500).json({ error: error?.message || "启动修复流程失败" });
  }
});
...
app.post("/api/agent/sessions/:sessionId/cancel", (req, res) => {
  try {
    const { sessionId } = req.params;
    const success = cancelAgentSession(sessionId);
...
app.get("/api/plans", (req, res) => {
  try {
    const plans = db.getAllFixPlans();
    res.json({ plans });
  } catch (error: any) {
    console.error("[Plans] Error:", error);
    res.status(500).json({ error: error?.message || "获取计划失败" });
  }
});
...
app.get("/api/plans/:planId", (req, res) => {
  try {
    ...
    const plan = db.getFixPlan(planId);
...
app.get("/api/skills", (req, res) => {
  try {
    ...
    res.status(500).json({ error: error?.message || "获取技能失败" });
  }
});
...
app.post("/api/skills", (req, res) => {
  try {
    ...
    const skill = db.createSkill({
      ...
      required_mcps: required_mcps ? JSON.stringify(required_mcps) : '[]',
      success_rate: 0.0,
      avg_duration: 0,
      created_at: now,
      updated_at: now,
    ...
    res.status(500).json({ error: error?.message || "创建技能失败" });
  }
});
...
app.get("/api/reflections", (req, res) => {
  try {
    ...
    if (sessionId) {
      const logs = db.getReflectionLogsBySession(sessionId as string);
      res.json({ logs });
    } else {
      // 获取所有日志（简化实现）
      res.json({ logs: [] });
    ...
    res.status(500).json({ error: error?.message || "获取反思日志失败" });
  }
});
...
app.get("/api/agent/health", (req, res) => {
  const agentSessions = getAllAgentSessions();
  const activeAgents = agentSessions.filter(s => s.state !== 'idle' && s.state !== 'error');
